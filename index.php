	...
	<?php if (have_posts()) : ?>
		...
		<?php while (have_posts()) : the_post(); ?>
			...
			<div class="post" id="post-<?php the_ID(); ?>">
				...
				<small>发表于<?php the_time('o年n月j日') ?> <!-- by <?php the_author() ?> --></small>

				<div class="entry">
					<?php the_content('阅读全文 &raquo;'); ?>
				</div>

				<p class="postmetadata"><!--发表于<?php the_category(', ') ?> | --><?php the_tags('标签: ', ', ', '&nbsp;| '); ?> <?php edit_post_link('编辑', '', ' | '); ?>  <?php comments_popup_link('没有评论&#187;', '1条评论 &#187;', '%条评论 &#187;'); ?></p>
			</div>
